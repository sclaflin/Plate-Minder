...
export default class RESTService {
	...
	constructor(port, enable) {
		...
		//curl -X PUT -H "Content-Type: application/json" -d '{"alwaysRestart": true}' localhost:5555/source/0/alwaysRestart
		this.app.put('/source/:index/alwaysRestart', (req, res) => this.updateSourceAlwaysRestart(req, res));
		...
		//curl -X PUT -H "Content-Type: application/json" -d '{"baseTopic": "plate-minder"}' localhost:5555/recorder/0/enableImage
		this.app.put('/recorder/:index/enableImage', (req, res) => this.updateRecorderEnableImage(req, res));
		...
		try {
			...
			switch(req.body.type) {
				case 'rtsp':
					...
					break;
				case 'mjpeg':
					source = MJPEGMJPEGReadable.fromObject(req.body);
					break;
				case 'file':
	...
	async updateSourceAlwaysRestart(req, res) {
		try {
			const { index } = req.params;
			const source = this.sources.get(index);
			const config = await this.readConfig();
			...
			const alwaysRestart = req.body.alwaysRestart;
			source.alwaysRestart = alwaysRestart;
			configSource.alwaysRestart = alwaysRestart;
	...
	async updateRecorderEnableImage(req, res) {
		try {
			const { index } = req.params;
			const recorder = this.recorders.get(index);
			const config = await this.readConfig();
			...
			const enableImage = req.body.enableImage;
			recorder.enableImage = enableImage;
			configRecorder.enableImage = enableImage;
