...
export default class PlateMinder {
	...
	constructor(sources, sinkClass, filters, openAlprDetect, recorders) {
		...
		if(sinkClass !== MJPEGToJPEG && MJPEGToJPEG.isPrototypeOf(sinkClass))
		...
		for(const source of this.sources) {
			...
			sink.on('jpeg', buffer => this.next(buffer));
			source.pipe(sink);
		}
	}
	get sources() {
	...
	async next(buffer) {
		...
		for(const filter of this.filters) {
			...
			if(filter.debug)
				await fs.writeFile(`${DATA_PATH}/${filter.constructor.name}.jpeg`, await filtered.toJpegBuffer());
		}
		...
		for(const recorder of this.recorders)
			await recorder.record(data, original, filtered);
