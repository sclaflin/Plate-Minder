...
export default class MQTTPlateRecorder extends PlateRecorder {
	...
	async record(data, source, original, filtered) {
		...
		for(const result of results) {
			...
			this.#client.publish(
				`${this.baseTopic}/${source.name}/timestamp`,
				Date.now().toString()
			);
		}
		this.#client.publish(
		...
		if(this.hassDiscovery.enable) {
			...
			this.#client.publish(
				`${this.hassDiscovery.discoveryPrefix}/sensor/${source.name.replace(/[^A-Za-z0-9_-]/, '_')}_timestamp/config`,
				JSON.stringify({
					availability: [
						{
							topic: `${this.baseTopic}/available`
					...
					state_topic: `${this.baseTopic}/${source.name}/timestamp`,
					unique_id: `plate-minder_${source.name.replace(/[^A-Za-z0-9_-]/, '_')}_timestamp`,
					name: `${source.name} Timestamp`
				})
			);
			this.#client.publish(
				`${this.hassDiscovery.discoveryPrefix}/image/${source.name.replace(/[^A-Za-z0-9_-]/, '_')}_image/config`,
				JSON.stringify({
					availability: [
						{
							topic: `${this.baseTopic}/available`
					...
					image_topic: `${this.baseTopic}/${source.name}/image`,
					unique_id: `plate-minder_${source.name.replace(/[^A-Za-z0-9_-]/, '_')}_image2`,
					name: `${source.name} Image`
				})
			);
			this.#client.publish(
				`${this.hassDiscovery.discoveryPrefix}/image/${source.name.replace(/[^A-Za-z0-9_-]/, '_')}_roi/config`,
				JSON.stringify({
					availability: [
						{
							topic: `${this.baseTopic}/available`
					...
					content_type: 'image/jpeg',
					image_topic: `${this.baseTopic}/${source.name}/roi`,
					unique_id: `plate-minder_${source.name.replace(/[^A-Za-z0-9_-]/, '_')}_roi2`,
					name: `${source.name} ROI`
				})
			);
		}
