...
export default class MQTTPlateRecorder extends PlateRecorder {
	...
	constructor(url, baseTopic, hassDiscovery, enableImage, mqttOptions) {
		super();
	...
	set enableImage(v) {
		if(typeof v !== 'boolean')
			throw new TypeError('enableImage must be a boolean.');
		this.#enableImage = v;
	}
	...
	async record(data, source, original, filtered) {
		...
		for(const result of results) {
			...
			if(this.enableImage || this.hassDiscovery.enable) {
				this.#client.publish(
